...
public class GameManager : MonoBehaviourPun
{
    ...
    void Update()
    {
        ...
        {
            Debug.Log(player_id);
            if(photonView.IsMine) //Solo si soy propietario del objeto al tomar input ejecuto esta linea
            {
                photonView.TransferOwnership(PhotonNetwork.PlayerList[player_id == 0 ? 1 : 0]); //Le doy al jugador contrario al posesion de algo. 
            }                                                                                   //Por default todo lo que tiene component photonView(que es para transeferir datos) lo tiene el host(jugador 1)
            else
            {
        ...
        {                                               //Se envia desde el qeu puse la tecla al servidor, y el serivdor ejecuta esta funcion en todos los clientes
            photonView.RPC("PassTurn", RpcTarget.All);  //Las variables son independientes de cada uno osea que hay que actualizarlas con un metodo como este.
